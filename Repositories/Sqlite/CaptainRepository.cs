...
public sealed class CaptainRepository : ICaptainRepository
{
    ...
    public CaptainRepository(DapperContext context)
    {
        _context = context;
    }
    ...
    public async Task<CaptainCredential?> GetByItsNumberAsync(string itsNumber, CancellationToken cancellationToken = default)
    {
        ...
        var query = "SELECT its_number AS ItsNumber, name AS Name, email AS Email, password_hash AS PasswordHash, new_password_hash AS NewPasswordHash FROM captains WHERE its_number = @ItsNumber LIMIT 1;";
